...
**It spends nothing, and that is structural.** Stage 2's keyless ceiling is
`maxCandidatesScored × maxLaunchesPerCandidate × maxRequestsPerLaunch` and the cost leg's is per
candidate SCORED; none of them is a function of how many candidates were ADMITTED, and
`maxCandidatesScored` does not move (captain decision 339a). The arm changes WHICH candidates
compete for a cap that does not move — 336a/399a's shape — and on that run the cap was not even
reached: 4 survivors against 7 slots.
...
typed the list. **A run that stops early says so on the same block**: the report's `CANDIDATE LIST`
page prints what the file held beside `coverage.gated` — what the run actually measured — and names
the shortfall outright when the two differ, so a list only part of which was gated cannot read as a
whole one.
...
- **COUNTERPARTY WALLET ADDRESSES USED TO BE DROPPED TOO, AND SINCE SCHEMA 24 THEY ARE KEPT**
  ...
  not the data, and 459's pivot to scoring entrants supplies it. `entry.windows[].entrants[]` now
  carries, per create-slot outsider, the wallet, its `sid`, its create-slot transaction, its queue
  position, what it staked, whether it closed and what it realised. **Nothing in the §5a(d) argument
  moves with it:** every one of those bytes comes from pump.fun's keyless public trade endpoint and
  from the chain, not from MadeOnSol — exactly as the creation walk's mints do, two bullets up — and
  no vendor per-token record is written at any schema version. **What genuinely changes is that a
  screened LAUNCH becomes identifiable from a committed record**, because a create slot plus an
  entrant address recovers the mint from the chain. That is on-chain public data either way and
  republishes no vendor row, but it is a real change of posture rather than a neutral one and it is
...
### Every position taken is counted, not only the ones that got out — captain decision 461

**The figures above are conditioned on the position having EXITED, and that is the OPTIMISTIC
construction rather than the conservative one.** A position that was entered and never exited was
dropped from the denominator rather than resolved, so `fieldHitRateGrossOfFees` and
`fieldHitRateNetOfMeasuredFees` are *P(profit | the position exited)* and not *P(profit)*. That is
the right quantity for the question Stage 2 asks — *is the field, once out, in profit* — and the
wrong one for *what did a position taken here actually realize*.

**Measured on the committed tape, over the same 32 launches and the same 265 create-slot outsider
positions, fee-inclusive:**

| construction | positive, exact 95% | net sum |
|---|---|---:|
| conditioned on exiting — every figure above | 80/158 = 0.5063 [0.4257, 0.5867] | **+108.28 SOL** |
| over every position taken | 86/265 = 0.3245 [0.2685, 0.3845] | **−8.12 SOL** |
| the same, with the two known winning units removed | 74/252 = 0.2937 [0.2382, 0.3541] | **−62.24 SOL** |

Same launches, same positions, one construction choice; the 107 that never got out are worth
−116.40 SOL between them. **It is a nested-subset comparison, not two pooled populations** — the 158
are a subset of the 265, which is what makes it legitimate. **And those positions are not unknowns,
they are losses:** of the 140 priced unexited entries, **7 = 0.0500 [0.0203, 0.1003]** are above
water once their remaining tokens are marked at the token's latest known price. They staked 267.19
SOL and are 149.17 SOL down in cash with 28.91 SOL of marked residual. Dropping them deletes the
losing half of the sample. (`slot-zero-stage3-exit-design` → `report.md` §§5.3, 5.4, held in
firstmate's records, not in this repo.)

**The rule: every position taken gets an outcome, and the three are kept apart.** `entry.mjs` →
`POSITION_OUTCOMES` owns it.

| outcome | what it means | how it is resolved |
|---|---|---|
| `exited` | Flat at the horizon, by the dataset's own closure rule | Its realized figure — the same one the `…OfFees` fields already carried |
| `still-held-at-horizon` | Bought, readable, and not flat | **ZERO RECOVERY** — the worst case — with the marked residual reported BESIDE it and never instead of it |
| `horizon-not-observed` | Our own rows cannot decide closure at all | **Neither way.** OUR COVERAGE: counted, surfaced, and **not filterable** under captain decision 174b |

`closedInWindow` is kept and is exactly `positionOutcome === 'exited'`, so no existing figure moves;
what it could not say is the difference between the second row and the third, and those are a fact
about the deployer's field and a fact about our own coverage respectively. `fieldOpenPositions` is
their sum, which is precisely the conflation this splits.

**Why zero recovery and not a mark.** A mark is a price nobody paid, and the captain's standing
evidence bar — *"everything measurable measured, everything not measurable given a firm boundary and
...
which way it runs is UNMEASURED — `entry.mjs` → `NET_ALL_POSITIONS_SELECTION_CAVEAT` owns the
argument, including why the two readings must not be differenced to infer a fee cost. **That scope
was deliberately not widened**: it would spend RPC requests this correction is not authorised to spend,
so the shortfall is stated by `fieldHitRateOverAllPositionsNetOfMeasuredFees.n` rather than closed.
Every rate added here carries its exact (Clopper–Pearson) interval as `lo`/`hi` beside `rate`;
`stats.mjs` holds the one implementation, shared with the 2026-08-10 measurement that first needed
it.

**It costs nothing and it gates nothing.** No vendor request, no credit, no wall clock, no wider
walk — every input was already in `readLaunchWindow`'s response and in `priceLaunchEntry`'s output,
and Stage 2's keyless ceiling (7 × 10 × 18 = 1,260) cannot move because none of the three factors is
a function of what is recorded. No bar, gate, threshold, predicate or verdict reads one of these
fields, no threshold moved, and a test pins both — the shape captain decision 208b established for
...
request count for the entrants' own costs, and it does **not** reach out-of-transaction tips as an
ATTRIBUTION, because attributing a sibling transaction in the same slot to the same bundle is an
inference the chain does not support. What it does buy, since captain decision 466, is a CEILING —
see the section below.

### The half of that block this tool used to throw away — captain decision 466

Stage 3 increment 2, record schema 26. When the block route serves a create slot, the same response
already holds **every other transaction in it**, and two of the cost terms that were blocking a
realized-profit verdict are sitting in there:

- **what it cost to TRY and fail** — every landed-but-FAILED transaction touching that launch's mint,
  with its exact `meta.fee`, base plus priority. Solana charges fees on inclusion rather than on
...
worst case is for — and it is the only reading the chain supports. What it replaces is a bracket
spanning three orders of magnitude on the cost of failing to land.

**Two refusals, both fail-safe.** No mint means no way to say which of a busy slot's failures were
attempts on this launch, so the observation is refused rather than widened to the whole slot; and a
walk that fell back to per-signature reads never saw the slot's other transactions, so it reports
...
an address missing from it makes the ceiling UNDER-state, which is the optimistic direction. That is
why a tip routed anywhere the list does not name sits in the ledger's UNBOUNDED row rather than being
folded into the bounded one.

### The subtraction ledger, and a verdict that is a function of it

`bounds.mjs` holds one typed row per cost and population component of a realized-profit figure —
`worstCaseSol` is a number where this build can bound the term and **`null` where it cannot** — and
`bounds.mjs` → `exitVerdict` is the captain's standing evidence bar compiled: *a cost component with
no numeric boundary blocks a profit verdict entirely.* It reaches the score's `caveats`, the run
record (`entry.costLedger`, `entry.exitVerdict`) and the rendered block, so the refusal travels with
the numbers rather than living here.

**It gates nothing.** No bar, threshold, predicate or entry verdict reads a row, `entry.verdict` is
byte-identical with the ledger present and absent, and no threshold moved — the shape captain
decision 208b established for `roomLeftBound`. `exitVerdict` is Stage 3's vocabulary and is never
`entry.verdict`'s.

**Three cost rows stay `null` after this increment**, so every candidate this build can score reads
`exit-unbounded` and increment 2 alone does **not** unlock a profit verdict for a general deployer:

| still unbounded | why |
|---|---|
| `landing-tip-outside-bound` | a tip paid in another slot of the window, or paid in the create slot by a route the pinned list does not name |
| `failed-attempts-rest-of-window` | pricing attempts in the other slots needs a whole-block read on up to `windowSlotSpan` slots per launch against `stage2_cost.maxRpcRequestsPerCandidate` for the WHOLE candidate — roughly 3x over for a single launch. **Captain decision 466 declined to raise it**, so this is deliberate |
| `exit-side-fees-outside-horizon` | a position closing after the walked horizon pays fees this walk never sees. Resolving it at zero recovery bounds what it RECOVERS, not what it spends recovering |

That last row is a third where `report.md` §6.6's prose says two: its own table gives that row `null`
today and no number after, so this build counts it as unbounded — the direction that refuses.

**Every verdict that states a result names its scope in its own name** — the binding consequence of
466. `exit-realised-at-worst-case-create-slot-costs-only`,
`exit-realised-typical-not-worst-case-create-slot-costs-only` and
`exit-loss-making-create-slot-costs-only`, so a reader who sees only the verdict string cannot take
it for a whole-window cost accounting. The two that report an ABSENCE of a result —`exit-unbounded`
and `exit-unmeasured` — carry no such suffix because they state no cost accounting, and they are
exactly the two a later stage may not filter on (captain decision 174b, one stage over).
