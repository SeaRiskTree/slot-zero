...
recovery **is** that worst case; `fieldResidualMarkedSolAtWindowLastPrice` is the bound on it, and
...
which for an unexited position means it made no transaction outside its create slot, since
`entryCostTargets` asks for a round-trip window only for wallets that closed. **That scope was
deliberately not widened**: it would spend RPC requests this correction is not authorised to spend,
